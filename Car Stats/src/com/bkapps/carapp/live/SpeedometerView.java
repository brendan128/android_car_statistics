...
//        setMeasuredDimension(width, height);
    ...
    private void drawNeedle(Canvas canvas) {
        RectF oval = getOval(canvas, 1.2f);
...
/*            	Log.v("inspeed", String.valueOf(minorStep));
                float angle = currentAngle + i*20;*/
            ...
            for (int i=1; i<=minorTicks; i=i+100) {
  //          	for (int i=1; i<=10; i++) {
                float angle = currentAngle + i*minorStep;
/*            	Log.v("inspeed", String.valueOf(minorStep));
                float angle = currentAngle + i*20;*/
            ...
            if (labelConverter != null) {
                ...
                float txtX = oval.centerX() + radius + majorTicksLength/2 + 18;
                float txtY = oval.centerY();
                canvas.rotate(+90, txtX, txtY);
                if (temp) {
                canvas.drawText(labelConverter.getLabelFor(curProgress-50, maxSpeed), txtX, txtY, txtPaint);
                } else {
    ...
    private void drawBackground(Canvas canvas) {
        RectF oval = getOval(canvas, 1.1f);
    ...
    private void init() {
       ...
       // needlePaint.setColor(Color.argb(200, 255, 0, 0));
        needlePaint.setColor(Color.rgb(102, 153, 0));
    }
